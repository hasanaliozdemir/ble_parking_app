...
class EditTpaPage extends StatefulWidget {
  final Tpa tpa;
  final Park park;
  const EditTpaPage({Key key, this.park, @required this.tpa}) : super(key: key);

  @override
  _EditTpaPageState createState() => _EditTpaPageState(park);
}

class _EditTpaPageState extends State<EditTpaPage> {
  final Park park;
  ...
  void initState() { 
    super.initState();
    _tpaNameController.text = widget.tpa.tpaName;
    _currentSelectedSize = widget.tpa.maxCarSize;
  }

  _EditTpaPageState(this.park);
  @override
  ...
  GestureDetector _buildDelete() {
    return GestureDetector(
      onTap: (){
        showDialog(context: context, builder: (context){
          return PopUp(title: "TPA Silinecek", 
          content: "Tekil Park Alanını silmek istediğinize emin misiniz?", 
          yesFunc: (){
                    _dataService.deleteTpa(
          tpaId: widget.tpa.tapId
        );
        Get.to(()=> ProfileScreen(),fullscreenDialog: true);
          ...
          realIcon: Icon(CupertinoIcons.trash,size: 48,),);
        });
      },
          child: Center(
            child: Text(
              "Tekil Park Alanını Sil",
              style: TextStyle(
                color: gray800
              ),
            ),
